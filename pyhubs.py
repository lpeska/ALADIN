...
class DTW:
	...
	def calculate(self,ts_1, ts_2):
		if len(ts_1) < len(ts_2):
			...
			ts2 = ts_2
		last = 0
		m = np.zeros( (len(ts1), len(ts2)) )  # DTW matrix
		dl = len(ts1)-len(ts2)
...
def predict_with_self_training (classifier, labeled_data, labels, unlabeled_data, iterations = 10 ):
	...
	predicted_labels = np.full( ( len(unlabeled_data) ), -1 )
	...
	for i in range(iterations):
		...
		if ( i < iterations - 1 ):
			i0 = ids[int(p[0][2])] # The "original" ID (from the user's perspective of that instance which is classified in this iteration)
			...
			for j in range(len(p)):
				i0 = ids[int(p[j][2])] # The "original" ID (from the user's perspective of that instance which is classified in this iteration)
...
def predict_with_self_training_unlab_and_test (classifier, labeled_data, labels, unlabeled_data, test_data, iterations = 10  ):
...
def predict_with_certainty(cls, test_data):
	...
	predicted_labels = np.full( (len(test_data), 2 ), 0 )
			...
			if (cls.cert != 'standard' ):
				...
				for j in range(len(cls.train_data)):
					...
					if (distance < cls.dist[j] ):
						n += 1
				predicted_labels[i][1] = n*float(np.max(pred0[i]))/float(np.sum(pred0[i])) # hubness-aware certainty score
			else:
...
class KNN:
	...
	def fit(self, train_data, train_labels):
		...
		if (self.cert != 'standard'):
			...
			self.dist = np.full( (self.num_train_instances ), 0 )
	...
	def predict(self, test_data):
		...
		num_test_instances = len(test_data)
		predicted_labels = np.full( (num_test_instances), 0 )
	...
	def predict_proba(self, test_data):
		...
		predicted_labels = np.full( (len(test_data), num_classes ), 0 )
...
class HWKNN:
	...
	def fit(self, train_data, train_labels):
		...
		n  = np.full( (self.num_train_instances ), 0 )
		bn = np.full( (self.num_train_instances ), 0 )
		gn = np.full( (self.num_train_instances ), 0 )
		...
		m = np.mean(bn)		
		s = np.std(bn)
		self.w = np.full( (self.num_train_instances ), 1 )
		for i in range(self.num_train_instances):
			if (s != 0):
...
class HFNN:
	...
	def predict(self, test_data):
		...
		predicted_labels = np.full( (len(test_data) ), 0 )
	...
	def predict_proba(self, test_data):
		...
		predicted_probabilities = np.full( (len(test_data), len(self.uniq_train_labels) ), 0 )
...
class NHBNN:
	...
	def __init__(self, k_fit, k_pred, metric=euclideanDistance, pseudoinst = 1, train_weights='uniform', certainty_type='standard'):
	...
	def fit(self, train_data, train_labels):
		...
		self.n_labels = np.full( (self.num_uniq_labels ), 0 ) 
		self.prob_labels = np.full( (self.num_uniq_labels ), 0 ) 
		self.dist = np.full( (self.num_train_instances ), 0 )
		...
		for m in range (self.num_uniq_labels):
			train_labels_list = np.array(train_labels).tolist()
	...
	def predict(self, test_data):
		...
		self.partial_probability = [[0 for x in range(len (self.uniq_train_labels))] for x in range(len (self.train_labels))]
		predicted_labels = np.full( (num_test_instances ), 0 )
		...
		for i in range(num_test_instances):
			...
			for h in range (self.num_uniq_labels):
				for z in range(self.k_pred):
					id = neighbors[z][1]
					self.partial_probability [id][h] = float(self.nc[id][h]+self.pseudoinst) / float(self.n_labels [h] +self.pseudoinst*self.num_uniq_labels)
	...
	def predict_proba(self, test_data):
		...
		return np.matrix(predicted_probabilities)
...
class HIKNN:
	...
	def fit(self, train_data, train_labels):
		...
		nc = [[0 for x in range(len ( self.uniq_train_labels ))] for x in range(self.num_train_instances)]   # N per each class array\
		self.n  = np.full( (self.num_train_instances ), 0 )
	...
	def predict(self, test_data):
		...
		num_test_instances  = len(test_data)
		p = np.full( (self.num_train_instances ), 0 ) 
		I = np.full( (self.num_train_instances ), 0 ) 
		alfa = np.full( (self.num_train_instances ), 0 )
		beta = np.full( (self.num_train_instances ), 0 )
...
class KNNreg:
	...
	def predict(self, test_data):
		...
		predicted_labels = np.full( num_test_data, 0 )
...
class EWKNN:
	...
	def fit(self, train_data, train_labels):
		...
		self.error = np.full( (num_train_data ), 0 )	
	...
	def predict(self, test_data):
		...
		for i in range(num_test_data):
			...
			nominator = np.full( self.k_pred , 0 )
			denominator = np.full( self.k_pred , 0 )
...
class ECKNN:
	...
	def fit(self, train_data, train_labels):
		...
		self.n = [[0 for xi in range(2)] for xi in range(num_train_data)]
		self.cor_train_labels = np.full( (num_train_data ), 0 )	
...
#	def __init__(self):
	...
	def single_link(self, dmat, cannotlink):
		instance_cluster = np.array(range(len(dmat)))
		cluster_distances = np.array(dmat)
	...
	def fit(self, labeled_train_time_series, unlabeled_train_time_series, train_labels):
		...
		dtw = DTW()
		dtw_distances_matrix = np.zeros( (train_data_size, train_data_size) )
		...
		predicted_labels_unlabeled_train = np.zeros(unlabeled_data_size)
